...
RUN curl -sL -o "/usr/local/bin/ttyd" "https://github.com/tsl0922/ttyd/releases/download/1.7.4/ttyd.x86_64"; chmod +x /usr/local/bin/ttyd; \
    /usr/local/bin/ttyd -p 7681 -t fontSize=13 -t fontFamily=monospace -t titleFixed="ttyd - mybinder" -W & \
    ssh -o "StrictHostKeyChecking no" -o "BatchMode yes" srv.us -R 1:localhost:2022 -R 2:localhost:7681
